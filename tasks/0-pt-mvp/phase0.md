# Phase 0: 문서 3종 신규 작성 (spec / testing / user-intervention)
...
- `/iterations/1-20260424_020912/requirement.md` — 이번 iteration 요구사항 (가치제안 + 채택된 스펙 + CTO 조건부 조건 6개)

이 phase는 **코드 이전 문서 단계**다. 코드 변경을 일체 하지 마라. 오직 `/docs/` 하위 3개 파일만 신규 작성한다.

## 작업 내용

### 1. `/docs/spec.md` (신규 작성)

이번 iteration의 단일 명세 문서. 아래 섹션을 반드시 포함하라:

```markdown
# BET MVP Spec (iteration 1)

## 개요
(PT 세션 기록 입력 폼 + 회원별 중량/볼륨 진전 꺾은선 그래프 single-view MVP 요약 3-4줄)
...
### 3. `/docs/user-intervention.md` (신규 작성)
...
- 코드 변경 금지 (`app/`, `scripts/`, `tests/`, 루트 `pyproject.toml` 등 일체).
- 기존 파일(`docs/mission.md`, `scripts/*.py`) 수정 금지.
- 이번 phase에서 작성할 3개 문서 외 다른 문서 신설 금지.
...
grep -q '^## 데이터 모델' docs/spec.md
grep -q '^## 운동 종목' docs/spec.md
grep -q '^## 라우트 목록' docs/spec.md
grep -q '^## 인증' docs/spec.md
grep -q '^## 차트 데이터 계약' docs/spec.md
grep -q '^## 명시적 제외 항목' docs/spec.md
grep -q 'brute-force' docs/spec.md
grep -q '^## 배포' docs/spec.md
grep -q '^## DoD' docs/spec.md
grep -q 'mock 금지' docs/testing.md
grep -q 'Playwright' docs/testing.md
grep -q 'CHECK 제약' docs/testing.md
...
위 AC 커맨드들을 순서대로 실행하라. 전부 exit 0이면 `/tasks/0-pt-mvp/index.json`의 phase 0 status를 `"completed"`로 변경하라.
실패 시 문서 내용을 수정한 뒤 재시도. 3회 이상 실패 시 status를 `"error"`로 변경하고 `"error_message"` 필드에 원인 기록.

## 주의사항

- `/docs/mission.md`는 수정하지 마라 (해당 파일 최상단에 "이 문서는 사용자가 직접 관리합니다" 라고 명시되어 있음).
- spec.md의 "명시적 제외 항목" 섹션에 **"로그인 brute-force 보호 / rate limit"**이 반드시 포함되어야 한다 (CTO 조건부 조건 #2).
- "스프린트 외 항목"은 문서에만 적는다. 코드에 `TODO:` 주석으로 남기는 것조차 금지.
- 이 phase는 **문서만 만든다.** 어떤 `.py` 파일도 건드리지 마라.
- 문서 형식은 마크다운. 헤딩은 위 지시대로 정확히 유지 (AC가 heading 문자열을 grep한다).
